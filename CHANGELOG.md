...
- Upgrade to Pest v5, Pest Browser Plugin v5, and PHPUnit 13.
- Upgrade the Pest type coverage plugin to v5.
- Use a single Composer autoload entrypoint for PestWP helpers and bootstrap.
- Update CI and documentation for the new compatibility baseline.

## [1.0.1] - 2026-01-09

### Fixed

- **Windows Support**: Check filesystem permissions before creating test directories in `AuthStateManager`, `SnapshotManager`, and `ScreenshotManager`
- **Screenshot Manager**: Fixed `imagecreatefrompng` warnings in tests by using valid PNG data
- **Expectations**: Added `WP_Error` support for `toHaveErrorCode` expectation
